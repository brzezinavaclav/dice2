...
$pages_ = ($perPage) ? $pocet/$perPage : 0;
    ...
    <table style="border: 0; border-collapse: collapse;">
      <tr>
        <td style="padding: 0; vertical-align: middle;">
          ...
          <big><?php echo sprintf("%.8f",$wallet->getbalance()); ?></big> <?php echo $settings['currency_sign']; ?>
          <br><br>
          <b>Free balance:</b><br>
          <big><?php $usersinv_=mysql_fetch_array(mysql_query("SELECT SUM(`amount`) AS `sum` FROM `investors` WHERE `amount`!=0")); $usersinv_['sum']=($settings['inv_enable']==1)?(0+(double)$usersinv_['sum']):0; $usersdeps_=mysql_fetch_array(mysql_query("SELECT SUM(`amount`) AS `sum` FROM `deposits`")); $usersdeps_['sum']=(0+(double)$usersdeps_['sum']);  $usersbal_=mysql_fetch_array(mysql_query("SELECT SUM(`balance`) AS `sum` FROM `players`")); $usersbal_['sum']=(0+(double)$usersbal_['sum']); echo sprintf("%.8f",$wallet->getbalance()-$usersbal_['sum']-$usersdeps_['sum']-$usersinv_['sum']); ?></big> <?php echo $settings['currency_sign']; ?>
        </td>
        <td style="vertical-align: middle;">
          <b>Reserved balance (users):</b><br>
          <big><?php echo sprintf("%.8f",$usersbal_['sum']+$usersinv_['sum']); ?></big> <?php echo $settings['currency_sign']; ?>
          <br><br>
          <b>Reserved deposits (users):</b><br>
          <big><?php echo sprintf("%.8f",$usersdeps_['sum']); ?></big> <?php echo $settings['currency_sign']; ?>        
        </td>
      </tr>
    </table>
  </div>
