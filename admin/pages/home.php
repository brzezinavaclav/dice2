  ...
  <fieldset style="margin-top: 7px;">
    ...
    <table class="vypis_table" style="width: 50%;">
      <tr class="vypis_table_obsah">
        ...
        <td><b><?php echo mysql_num_rows(mysql_query("SELECT `id` FROM `investors` WHERE `amount`!=0")); ?></b></td>
      </tr>
      <tr class="vypis_table_obsah">
        <td title="Total invested by investors">Total Invested:</td>
        <td><b><?php $tsum=mysql_fetch_array(mysql_query("SELECT SUM(`amount`) AS `am` FROM `investors` WHERE `amount`!=0")); echo sprintf("%.8f",$tsum['am']); ?></b> <?php echo $settings['currency_sign']; ?></td>
      </tr>
      <tr class="vypis_table_obsah">
        <td title="= free balance">House Investment:</td>
        <td><b><?php $usersinv_=mysql_fetch_array(mysql_query("SELECT SUM(`amount`) AS `sum` FROM `investors` WHERE `amount`!=0")); $usersinv_['sum']=($settings['inv_enable']==1)?(0+(double)$usersinv_['sum']):0; $usersdeps_=mysql_fetch_array(mysql_query("SELECT SUM(`amount`) AS `sum` FROM `deposits`")); $usersdeps_['sum']=(0+(double)$usersdeps_['sum']);  $usersbal_=mysql_fetch_array(mysql_query("SELECT SUM(`balance`) AS `sum` FROM `players`")); $usersbal_['sum']=(0+(double)$usersbal_['sum']); echo sprintf("%.8f",$wallet->getbalance()-$usersbal_['sum']-$usersdeps_['sum']-$usersinv_['sum']); ?></b> <?php echo $settings['currency_sign']; ?></td>
      </tr>
      <tr class="vypis_table_obsah">
        <td>Total Investor's Profit:</td>
        <td><b><?php $tsum=mysql_fetch_array(mysql_query("SELECT SUM(`profit`) AS `am` FROM `investors` WHERE `profit`!=0")); if ($tsum['am']<0) echo '<span style="color: #d10000;">'.sprintf("%.8f",$tsum['am']).'</span>'; else echo '<span style="color: green;">'.sprintf("%.8f",$tsum['am']).'</span>'; ?></b> <?php echo $settings['currency_sign']; ?></td>
      </tr>
      <tr class="vypis_table_obsah">
