...
if (isset($included) && $logged==true) {
  ...
  if (isset($_POST['addons_form'])) {
    ...
    mysql_query("UPDATE `system` SET `inv_min`=".max(0,(double)$_POST['inv_min']).",`inv_perc`=".max(0,min((int)$_POST['inv_perc'],100)).",`giveaway`=$giveaway,`giveaway_amount`=".(double)$_POST['giveaway_amount'].",`giveaway_freq`=".(int)$_POST['giveaway_freq'].",`chat_enable`=$chat_enable,`bot_enable`=$bot_enable,`inv_enable`=$inv_enable LIMIT 1");
  }
